...
fileQueue.process(async (job, done) => {
   ...
   const file = await ( await dbClient.fileCollection())
     .findOne({
...
userQueue.process(async (job, done) => {
  ...
  try {
    const mailSubject = 'Welcome to ALX-Files_Manager by Codex';
    const mailContent = [
      '<div>',
      '<h3>Hello {{user.name}},</h3>',
      'Welcome to <a href="https://github.com/mathewCodex/alx-files_manager">',
      'ALX-Files_Manager</a>, ',
      'a simple file management API built with Node.js by ',
      '<a href="https://github.com/mathewCodex">Mathew Emmanuel</a>. ',
